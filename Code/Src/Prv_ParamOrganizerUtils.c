...
e_paramOrgResult getPagePrmFromRamBuff(s_paramOrgContext* prmCntx, uint8_t* page, s_prv_paramOrgPageParam* prmPage)
{
    ...
    else
    {
        ...
        else
        {
            ...
            (void)memcpy( (uint8_t*)&prmPage->pageTimeSetted,         &page[offset1], sizeof(uint32_t) );
            (void)memcpy( (uint8_t*)&prmPage->pageType,               &page[offset2], sizeof(uint8_t)  );
            (void)memcpy( (uint8_t*)&prmPage->allPageAlignmentNumber, &page[offset3], sizeof(uint8_t)  );
            (void)memcpy( (uint8_t*)&prmPage->pageVersion,            &page[offset4], sizeof(uint16_t) );
            (void)memcpy( (uint8_t*)&prmPage->pageMagicNumber,        &page[offset5], sizeof(uint32_t) );
            (void)memcpy( (uint8_t*)&prmPage->pageCrc,                &page[offset6], sizeof(uint32_t) );
...
e_paramOrgResult setPagePrmInRamBuff(s_paramOrgContext* prmCntx, uint8_t* page, s_prv_paramOrgPageParam* prmPage)
{
    ...
    else
    {
        ...
        else
        {
            ...
            (void)memcpy( &page[offset1], (uint8_t*)&prmPage->pageTimeSetted,           sizeof(uint32_t) );
            (void)memcpy( &page[offset2], (uint8_t*)&prmPage->pageType,                 sizeof(uint8_t)  );
            (void)memcpy( &page[offset3], (uint8_t*)&prmPage->allPageAlignmentNumber,   sizeof(uint8_t)  );
            (void)memcpy( &page[offset4], (uint8_t*)&prmPage->pageVersion,              sizeof(uint16_t) );
            (void)memcpy( &page[offset5], (uint8_t*)&prmPage->pageMagicNumber,          sizeof(uint32_t) );
            (void)memcpy( &page[offset6], (uint8_t*)&prmPage->pageCrc,                  sizeof(uint32_t) );
...
e_paramOrgResult setPagePrmInRamBuffNCrcUp(s_paramOrgContext* prmCntx, uint8_t* page, s_prv_paramOrgPageParam* prmPage)
{
    ...
    else
    {
        ...
        else
        {
            ...
            /* Now we can calculate the CRC of the page, ecluding the CRC itself */
            /* Esclude from the calculation the CRC, that is placed in the last 4 byte */
            pageCrcSizeToCalc = prmCntx->pageSize - sizeof(uint32_t);
            if( false == (*(prmCntx->pToCrcFunc))(&crcCalculated, page, pageCrcSizeToCalc, CRC_BASE_SEED) )
            {
...
e_paramOrgResult setCrcInPagePrmRamBuff(s_paramOrgContext* prmCntx, uint8_t* page, uint32_t* crcToSet)
{
    ...
    else
    {
        ...
        else
        {
            ...
            uint32_t offset6 = prmCntx->pageSize - ( sizeof(uint32_t) );
            (void)memcpy( &page[offset6], (uint8_t*)crcToSet, sizeof(uint32_t) );
            returnVal = PARAMRES_ALLOK;
        }
    }
        ...
        else
        {
            ...
            if( PARAMRES_ALLOK == returnVal )
            {
                ...
                if( PARAMRES_ALLOK == returnVal )
                {
                    ...
                    {
                        /* Verify if the page type is what we are aspecting */
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if( false == (*(prmCntx->pToWriteFunc))(prmCntx->pageId, pageOffset, prmCntx->pageSize, prmCntx->memPoolPointer) )
                    {
                        returnVal = PARAMRES_ERRORREAD;
                    }
                    else
